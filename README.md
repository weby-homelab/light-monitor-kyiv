# 🇺🇦 Light Monitor Kyiv (Extended)

[![Check Outages](https://img.shields.io/github/actions/workflow/status/weby-homelab/light-monitor-kyiv/check_outages.yml?branch=main&label=Check%20Outages&style=flat-square)](https://github.com/weby-homelab/light-monitor-kyiv/actions/workflows/check_outages.yml)
[![Python](https://img.shields.io/badge/python-3.10%2B-blue?style=flat-square)](https://www.python.org/)
[![License](https://img.shields.io/github/license/weby-homelab/light-monitor-kyiv?style=flat-square)](LICENSE)

Це розширена система моніторингу електропостачання для Києва (та інших регіонів, що підтримуються Yasno/GitHub джерелами).
Проект не просто показує графіки, а **порівнює обіцянки енергетиків з реальністю**, генерує красиві звіти та миттєво сповіщає про зміни.

---

## 🌟 Основні можливості

### 1. 🔍 Моніторинг реального стану (Power Monitor)
*   **Миттєві сповіщення:** Надсилає повідомлення `🔴 Світло зникло` та `🟢 Світло з'явилося` за секунди після події.
*   **Контекст графіків:** Бот розуміє поточний графік і коментує події:
    *   *"Світло зникло, хоча за графіком мало бути ще 30 хв"*
    *   *"Світло з'явилося раніше на 15 хв"*
*   **Статистика відключення:** При появі світла пише, скільки часу його не було.

### 2. 📊 Щоденна аналітика "План vs Факт"
Щоночі генерується графічний звіт у темній темі:
*   **Візуалізація:** Дві часові шкали (Планова та Фактична) одна над одною.
*   **Точна статистика:**
    *   Скільки годин світла було реально.
    *   Скільки обіцяли за графіком.
    *   **Відхилення:** Наприклад, *"-2.5 год (виконання плану 85%)"*.

### 3. 📅 Тижневі звіти
Щотижня (у понеділок вночі) надходить великий звіт:
*   **Енергетичний "Вердикт":** Текстовий аналіз стабільності тижня (від "Спокійний" до "Важкий").
*   **Інфографіка:** Зведена діаграма за 7 днів.
*   **Рейтинг днів:** Визначення найкращого та найгіршого дня тижня.

### 4. 🤖 Автоматичне оновлення графіків
*   Скрипт постійно перевіряє API Yasno та GitHub (дані від спільноти).
*   При зміні графіку (наприклад, скасування відключень) надсилає сповіщення.

---

## 🛠 Компоненти системи

Система складається з чотирьох незалежних скриптів, які працюють в одній папці:

1.  `power_monitor_server.py`: HTTP-сервер, який слухає "heartbeat" від вашого роутера/пристрою вдома. Відповідає за `🔴/🟢` сповіщення.
2.  `main.py`: Скрейпер графіків. Оновлює `last_schedules.json` та історію `schedule_history.json`.
3.  `generate_daily_report.py`: Генерує картинку звіту за вчора.
4.  `generate_weekly_report.py`: Генерує звіт за тиждень.

---

## 🚀 Встановлення

Вам знадобиться VPS або пристрій, що працює 24/7 (Raspberry Pi), та налаштований "маячок" вдома (наприклад, скрипт на роутері, що смикає URL при наявності інтернету).

### 1. Підготовка
```bash
git clone https://github.com/weby-homelab/light-monitor-kyiv.git
cd light-monitor-kyiv
python3 -m venv venv
source venv/bin/activate
pip install -r requirements.txt
```

### 2. Конфігурація (.env)
Створіть файл `.env`:
```ini
TELEGRAM_BOT_TOKEN=123456:ABC-DEF1234ghIkl-zyx57W2v1u123ew11
TELEGRAM_CHANNEL_ID=-1001234567890
```

### 3. Налаштування груп (config.json)
Відредагуйте `config.json` під вашу чергу (приклад для 6 групи):
```json
{
  "settings": {
    "region": "kiev",
    "groups": ["GPV36.1"], 
    "style": "list"
  },
  ...
}
```

### 4. Запуск серверу моніторингу
Запустіть сервер у фоні (або створіть systemd service):
```bash
./venv/bin/python power_monitor_server.py
```
Він створить унікальний URL для вашого пристрою (див. консоль або `power_push_url.txt`). Налаштуйте ваш домашній роутер (або інший пристрій) робити `GET` запит на цей URL кожну хвилину.

### 5. Налаштування Cron (Розклад)
Додайте задачі в `crontab -e`:

```cron
# 1. Оновлення графіків (щогодини)
0 * * * * cd /path/to/project && ./venv/bin/python main.py >> cron_scraper.log 2>&1

# 2. Щоденний звіт (00:05)
5 0 * * * cd /path/to/project && export $(grep -v '^#' .env | xargs) && ./venv/bin/python generate_daily_report.py >> cron_daily.log 2>&1

# 3. Тижневий звіт (Понеділок 00:10)
10 0 * * 1 cd /path/to/project && export $(grep -v '^#' .env | xargs) && ./venv/bin/python generate_weekly_report.py >> cron_weekly.log 2>&1
```

---

## 📊 Приклад звітів

### Щоденний:
> 📊 **Звіт за 12.02.2026**
> 💡 Світло було: **19год 15хв**
> ❌ Світла не було: **4год 44хв**
>
> 📉 **План vs Факт:**
> • За планом світло: **10год 00хв**
> • Реально світло: **19год 15хв**
> • Відхилення: **+9.2год** (Світла **192%** від плану)

---

## 📂 Файли даних

*   `event_log.json`: Історія всіх включень/вимкнень (формується автоматично).
*   `schedule_history.json`: Архів планових графіків (для аналітики).
*   `last_schedules.json`: Поточний актуальний графік.

---

## 🤝 Подяка

*   [banditByte](https://github.com/banditByte) за основу парсера графіків.
*   Спільноті [outage-data-ua](https://github.com/Baskerville42/outage-data-ua) за API графіків.

---
**License:** MIT
