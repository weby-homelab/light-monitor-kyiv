...
### 3. 🌐 Web Dashboard (v1.2.0)
Вбудований веб-інтерфейс для перегляду статусу без Telegram (`http://IP:8889/`):
*   **Live-Status:** Великий індикатор (Є/Немає) та таймер тривалості.
*   **Історія:** Останні 10 подій (увімкнення/вимкнення) у вигляді таблиці.
*   **Графіки:** Актуальні діаграми за **Сьогодні** та **Тиждень** (оновлюються автоматично).
*   **Аналітика:** Блок "План vs Факт" у реальному часі.
*   Ідеально для виводу на планшет, старий телефон або "розумне дзеркало".
...
# Live-звіт та Web Dashboard (кожні 10 хв)
*/10 * * * * /root/geminicli/light-monitor-kyiv/update_web_charts.sh >> /root/geminicli/light-monitor-kyiv/cron.log 2>&1
...
## 🆕 Release Notes v1.2.0 (Energy Control Center)

Це велике оновлення фокусується на покращенні Web Dashboard, перетворюючи його на повноцінний центр керування енергопостачанням.

### 🌟 Що нового:
1.  **Розширений Web Dashboard:**
    *   **Історія подій:** Додано таблицю останніх 10 подій з розрахунком тривалості попереднього стану (наприклад, "Світло з'явилося (не було 4 год 10 хв)").
    *   **Тижневий графік:** Тепер на дашборді відображається не тільки денний, а й тижневий графік відключень.
    *   **Чистий дизайн:** Оновлено футер та покращено відображення на мобільних пристроях.

2.  **Покращена Автоматизація:**
    *   **Швидке оновлення:** Графіки для веб-панелі тепер оновлюються кожні **10 хвилин** (раніше 30 хв).
    *   **Миттєва реакція:** При зміні статусу (зникнення/поява світла) веб-панель оновлюється автоматично.

3.  **Скриншот Web Dashboard:**
    ![Web Dashboard v1.2.0](docs/img/web_dashboard.jpg)
    *(Приклад вигляду панелі v1.2.0)*
